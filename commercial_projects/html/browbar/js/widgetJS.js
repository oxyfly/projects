	...
	var yWidget = {
		id: '97992',
		href: 'https://n97992.yclients.com/',
        script_href: 'https://w97992.yclients.com/',
		lang: 'rus',
		isNew: '1',
	...
	createButton: function() {
        ...
        var isHasCookie = function (cookie) {
            return cookie.indexOf('yextrafield_') !== -1;
        };

        var getYextraCookie = function () {
            return document.cookie.split(';').reduce(function (cookies, cookie) {
                var [ name, value ] = cookie.split('=').map(function (c) { return c.trim() });

                if (isHasCookie(name) === true) {
                    cookies[name] = value;
                }
        ...
        if (isHasCookie(document.cookie) === true) {
            var cookies = getYextraCookie();
            var resultSearch = '';
            var i = 0;

            for (var k in cookies) {
                if(cookies.hasOwnProperty(k)) {
                    resultSearch += k + '=' + cookies[k] + (i !== (Object.keys(cookies).length - 1) ? '&' : '');
                }

                i += 1;
            }

            const beforeSearch = document.location.search === '' ? '?' : '&';

            button.search = beforeSearch + resultSearch;
        }
